...
public class Auxiliar {
    ...
    public static List<Proceso> LoadFile(File file) {
        ...
        try {
            Auxiliar.pathOut = file.getAbsolutePath();
    ...
    public static void imprimir(SO so) {
        if (!isDebug) {
            System.out.println("\n\n---------------------------------------------------");
    ...
    public static void debug(SO so) {
    ...
    private static void printFile(String line) {
        if(null == Auxiliar.out)
            out = Auxiliar.pathOut+"_"+System.currentTimeMillis()+".txt";
        try {
